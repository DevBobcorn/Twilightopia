...
public class MistyTaigaTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   ...
   public MistyTaigaTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i51429_1_, boolean p_i51429_2_) {
      super(p_i51429_1_, p_i51429_2_);
      setSapling((net.minecraftforge.common.IPlantable)ModBlocks.PROPHET_SAPLING); //Just 2 make it plant-able on Chocolate
   }
   ...
   public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox p_208519_5_) {
      ...
      if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getMaxHeight()) {
         ...
         if (!flag) {
            return false;
         } else if (TwilightopiaSoilHelper.isSoil(worldIn, position.down()) && position.getY() < worldIn.getMaxHeight() - i - 1) {
