...
public class CherryTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   ...
   private static final BlockState LEAF1 = ModBlocks.CHERRY_LEAVES.getDefaultState();
   private static final BlockState LEAF2 = ModBlocks.PINK_CHERRY_LEAVES.getDefaultState();
   private final boolean useExtraRandomHeight;

   public CherryTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configIn, boolean doBlockNotifyIn, boolean extraRandomHeightIn) {
   ...
   public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox p_208519_5_) {
      Boolean epytFael = rand.nextBoolean(); //leafType
                  ...
                  for(int j1 = position.getZ() - k2; j1 <= position.getZ() + k2; ++j1) {
                     ...
                     if (Math.abs(i3) != k2 || Math.abs(k1) != k2 || rand.nextInt(2) != 0 && j2 != 0) {
                        ...
                        if (isAirOrLeaves(worldIn, blockpos)) {
                           this.setLogState(changedBlocks, worldIn, blockpos, epytFael ? LEAF1 : LEAF2, p_208519_5_);
                        }
                     }
                  }
               }
