...
@OnlyIn(Dist.CLIENT)
public class SignPlusTileEntityRenderer extends TileEntityRenderer<SignPlusTileEntity> {
   private static final ResourceLocation cherry = new ResourceLocation(TwilightopiaMod.MODID, "textures/entity/signs/cherry.png");
   private static final ResourceLocation ignite = new ResourceLocation(TwilightopiaMod.MODID, "textures/entity/signs/ignite.png");
   private final SignModel model = new SignModel();

   public void render(SignPlusTileEntity tileEntityIn, double x, double y, double z, float partialTicks, int destroyStage) {
      BlockState blockstate = tileEntityIn.getBlockState();
      GlStateManager.pushMatrix();
      if (blockstate.getBlock() instanceof StandingSignPlusBlock) {
         GlStateManager.translatef((float)x + 0.5F, (float)y + 0.5F, (float)z + 0.5F);
         GlStateManager.rotatef(-((float)(blockstate.get(StandingSignPlusBlock.ROTATION) * 360) / 16.0F), 0.0F, 1.0F, 0.0F);
         this.model.getSignStick().showModel = true;
      } else {
         GlStateManager.translatef((float)x + 0.5F, (float)y + 0.5F, (float)z + 0.5F);
         GlStateManager.rotatef(-blockstate.get(WallSignPlusBlock.FACING).getHorizontalAngle(), 0.0F, 1.0F, 0.0F);
   ...
   private ResourceLocation getTexture(Block p_217658_1_) {
      if (p_217658_1_ == ModBlocks.CHERRY_SIGN || p_217658_1_ == ModBlocks.CHERRY_WALL_SIGN) {
    	 return cherry;
      } else {
         return ignite;
      }
