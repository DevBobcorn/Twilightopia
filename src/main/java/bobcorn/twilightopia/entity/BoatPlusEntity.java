...
public class BoatPlusEntity extends Entity {
   private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.FLOAT);
   private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(BoatPlusEntity.class, DataSerializers.VARINT);
   ...
   private BoatPlusEntity.Status status;
   private BoatPlusEntity.Status previousStatus;
   ...
   public BoatPlusEntity(EntityType<? extends BoatPlusEntity> p_i50129_1_, World p_i50129_2_) {
      super(p_i50129_1_, p_i50129_2_);
      this.preventEntitySpawning = true;
   }

   public BoatPlusEntity(World worldIn, double x, double y, double z) {
      //this(ModEntityType.BOAT_PLUS, worldIn);
	  this(null, worldIn);
   ...
   protected void registerData() {
      ...
      this.dataManager.register(BOAT_TYPE, BoatPlusEntity.Type.CHERRY.ordinal());
...
public boolean attackEntityFrom(DamageSource source, float amount) {
   ...
   public void applyEntityCollision(Entity entityIn) {
      if (entityIn instanceof BoatPlusEntity) {
...
public boolean canBeCollidedWith() {
   ...
   public void tick() {
      ...
      if (this.status != BoatPlusEntity.Status.UNDER_WATER && this.status != BoatPlusEntity.Status.UNDER_FLOWING_WATER) {
    ...
    */
   private BoatPlusEntity.Status getBoatStatus() {
      BoatPlusEntity.Status boatentity$status = this.getUnderwaterStatus();
      if (boatentity$status != null) {
         this.waterLevel = this.getBoundingBox().maxY;
         return boatentity$status;
      } else if (this.checkInWater()) {
         return BoatPlusEntity.Status.IN_WATER;
      } else {
         float f = this.getBoatGlide();
         if (f > 0.0F) {
            this.boatGlide = f;
            return BoatPlusEntity.Status.ON_LAND;
         } else {
            return BoatPlusEntity.Status.IN_AIR;
         }
      }
   }
    ...
    */
   @Nullable
   private BoatPlusEntity.Status getUnderwaterStatus() {
      AxisAlignedBB axisalignedbb = this.getBoundingBox();
               ...
               for(int i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutableblockpos.getY() + ifluidstate.func_215679_a(this.world, blockpos$pooledmutableblockpos))) {
                     if (!ifluidstate.isSource()) {
                        BoatPlusEntity.Status boatentity$status = BoatPlusEntity.Status.UNDER_FLOWING_WATER;
                        return boatentity$status;
                     }
      ...
      return flag ? BoatPlusEntity.Status.UNDER_WATER : null;
   }
   ...
   private void updateMotion() {
      ...
      double d2 = 0.0D;
      this.momentum = 0.05F;
      if (this.previousStatus == BoatPlusEntity.Status.IN_AIR && this.status != BoatPlusEntity.Status.IN_AIR && this.status != BoatPlusEntity.Status.ON_LAND) {
         ...
         this.lastYd = 0.0D;
         this.status = BoatPlusEntity.Status.IN_WATER;
      } else {
         if (this.status == BoatPlusEntity.Status.IN_WATER) {
            d2 = (this.waterLevel - this.getBoundingBox().minY) / (double)this.getHeight();
            this.momentum = 0.9F;
         } else if (this.status == BoatPlusEntity.Status.UNDER_FLOWING_WATER) {
            d1 = -7.0E-4D;
            this.momentum = 0.9F;
         } else if (this.status == BoatPlusEntity.Status.UNDER_WATER) {
            d2 = (double)0.01F;
            this.momentum = 0.45F;
         } else if (this.status == BoatPlusEntity.Status.IN_AIR) {
            this.momentum = 0.9F;
         } else if (this.status == BoatPlusEntity.Status.ON_LAND) {
   ...
   protected void readAdditional(CompoundNBT compound) {
      if (compound.contains("Type", 8)) {
         this.setBoatType(BoatPlusEntity.Type.getTypeFromString(compound.getString("Type")));
      }
...
protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
      ...
      if (!this.isPassenger()) {
         if (onGroundIn) {
            if (this.fallDistance > 3.0F) {
               if (this.status != BoatPlusEntity.Status.ON_LAND) {
   ...
   public void setBoatType(BoatPlusEntity.Type boatType) {
      this.dataManager.set(BOAT_TYPE, boatType.ordinal());
   }

   public BoatPlusEntity.Type getBoatType() {
      return BoatPlusEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
   }
   ...
   public static enum Type {
      CHERRY(ModBlocks.CHERRY_PLANKS, "cherry"),
      IGNITE(ModBlocks.IGNITE_PLANKS, "ignite");
       ...
       */
      public static BoatPlusEntity.Type byId(int id) {
         BoatPlusEntity.Type[] aboatentity$type = values();
      ...
      public static BoatPlusEntity.Type getTypeFromString(String nameIn) {
         BoatPlusEntity.Type[] aboatentity$type = values();
