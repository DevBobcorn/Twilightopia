...
public final class FoggyForestBiome extends Biome {
	public FoggyForestBiome() {
		super((new Biome.Builder())
				.surfaceBuilder(SurfaceBuilder.DEFAULT, TwilightopiaSurfaceBuilder.MISTY_CHOCO_CONFIG)
				.precipitation(Biome.RainType.RAIN).category(Biome.Category.FOREST).depth(0.125F).scale(0.05F)
				.temperature(0.6F).downfall(0.5F).waterColor(TwilitColors.misty2).waterFogColor(TwilitColors.misty1).parent((String) null));
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
				createDecoratedFeature(ModFeature.MISTY_BIRCH_TREE, IFeatureConfig.NO_FEATURE_CONFIG,
						Placement.COUNT_EXTRA_HEIGHTMAP, new AtSurfaceWithExtraConfig(1, 1.0F, 1)));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
		...
		this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(ModEntityType.DEER, 10, 4, 4));
		...
		this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.WITHER_SKELETON, 5, 1, 4));
	}
