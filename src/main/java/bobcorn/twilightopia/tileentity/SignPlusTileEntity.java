...
import bobcorn.twilightopia.client.gui.EditSignPlusScreen;
...
public class SignPlusTileEntity extends TileEntity {
	public final ITextComponent[] signText = new ITextComponent[] { new StringTextComponent(""),
	...
	public SignPlusTileEntity() {
		super(ModTileEntityType.SIGN_PLUS);
	}
	...
	public CompoundNBT write(CompoundNBT compound) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			//System.out.println("Write Line " + i + ": " + s);
	...
	public CommandSource getCommandSource(@Nullable ServerPlayerEntity playerIn) {
		...
		return new CommandSource(ICommandSource.field_213139_a_,
				...
				Vec2f.ZERO, (ServerWorld) this.world, 2, s, itextcomponent, this.world.getServer(), playerIn);
	}
	...
	// @OnlyIn(Dist.CLIENT) Makes it so this method will be removed from the class
	// on the PHYSICAL SERVER
	// This is because we only want to handle opening the GUI on the physical
	// client.
	@OnlyIn(Dist.CLIENT)
	public void openGuiClient(SignPlusTileEntity sign) {
		//System.out.println("gui - client!");
		Minecraft.getInstance().displayGuiScreen(new EditSignPlusScreen(sign));
	}
	
	public void openGuiServer(ServerPlayerEntity player,SignPlusTileEntity sign) {
		//System.out.println("gui - server!");
		sign.setPlayer(player);
		//player.connection.sendPacket(new SOpenSignPlusMenuPacket(sign.getPos()));
		//Minecraft.getInstance().getConnection().getNetworkManager().sendPacket(new SOpenSignMenuPacket(sign.getPos()));
		//System.out.println("Send S2C, Open Edit Gui!");
		TwilightopiaMod.CHANNEL.send(PacketDistributor.ALL.noArg(), new S2CEditSignPlus(sign.getPos()));
	}
}